...
| # | 挑战 | 传统方法的局限 |
|---|---|---|
| C1 | **质量进入优化目标**：计划选择不仅影响成本和延迟，还可能影响结果质量 | 传统查询优化通常以保持结果语义不变为前提；AI 查询优化还需要显式处理质量约束 |
| C2 | **不同后端具有异构的成本-延迟-质量特征**：embedding / proxy / oracle / 专用模型对应不同的执行成本、延迟和质量水平 | 优化器需要比较不同模型后端及其级联组合，单一的算子代价参数难以完整描述这些选项 |
| C3 | **等价变换需要新的判定条件**：LLM 的非确定性、近似实现和语义重写可能改变结果分布 | 传统重写依赖确定的算子语义和代数等价律；AI 算子需要额外定义并验证近似程度 |
| C4 | **可重复性与一致性语义需重新定义**：缓存策略、模型更新和事务语义相互影响 | 在非确定算子下，传统的可重复读语义难以直接保证 |

**Ⅱ. 优化器决策**

| # | 挑战 | 传统方法的局限 |
|---|---|---|
| C5 | **缺少可复用的编译期统计信息**：选择率、模型成本和错误率依赖数据分布与模型版本，难以直接由传统统计目录提供 | 现有方法常通过试跑获取这些信息，但尚缺少跨查询积累和复用的机制（详见 3.7） |
| C6 | **部分优化决策需要运行期反馈**：选择率、模型延迟和级联阈值等信息可能只有在执行过程中才能获得 | 传统优化主要依赖编译期统计信息，AI 算子则需要根据试跑或在线反馈调整执行决策 |
| C7 | **搜索空间扩大**：放置决策的复杂度随语义算子数指数增长（PLOP 的 DP 为 O(3ⁿ)） | 除 join 顺序外，优化器还需联合考虑 AI 算子的放置和物理实现 |
| C8 | **质量约束缺少查询级声明机制**：用户需要在查询中声明可接受的质量损失 | SQL 缺少表达近似正确性的标准语法；各系统主要通过自定义 API 提供该能力 |

**Ⅲ. 执行与资源管理**

| # | 挑战 | 传统方法的局限 |
|---|---|---|
| C9 | **优化开销不可忽略**：采样试跑和 AI oracle 判定都会产生模型调用成本 | 对于短查询，优化开销可能接近或超过优化所带来的收益 |
| C10 | **执行资源约束增加**：GPU 配额、推理并发度和 API rate limit 都会影响计划执行 | 传统资源模型主要考虑 CPU、内存和 I/O，难以直接表达模型服务的资源约束 |


**小结**：现有工作大多只形式化了部分决策，例如 PLOP 主要研究放置，Palimpzest 主要研究实现选择，级联方法主要针对单个算子。目前尚缺少同时覆盖 T、π、ι 和 ρ，并联合考虑成本、质量、延迟与统计不确定性的统一形式化。
