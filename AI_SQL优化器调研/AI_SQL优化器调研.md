...
| # | 挑战 | 传统方法的局限 |
|---|---|---|
| C1 | **统计量难以估计**：σ、c、ε 难以直接通过直方图估计，并会随模型版本和数据分布变化；在级联执行中还需估计整体的质量-成本曲线 | 传统优化器依赖可从统计目录获取的估计（详见 3.7） |
| C2 | **等价变换的条件发生变化**：LLM 输出可能具有非确定性，近似实现和语义重写也可能改变结果分布 | 传统重写主要依赖确定的算子语义和代数等价律；AI 算子需要额外定义并验证近似程度 |
| C3 | **质量成为优化目标**：更换物理实现可能同时改变执行性能和结果质量 | 传统物理实现选择通常保持查询结果不变，而 AI 算子需要在成本、延迟和质量之间权衡 |
| C4 | **物理实现异构且影响质量**：embedding / proxy / oracle / 专用模型具有不同的成本和质量特性 | 传统的物理实现虽然也有离散选项，但通常不改变结果语义；AI 算子的实现选择还会影响输出质量 |
| C5 | **搜索空间扩大**：放置决策的复杂度随语义算子数指数增长（PLOP 的 DP 为 O(3ⁿ)） | 除 join 顺序外，优化器还需联合考虑 AI 算子的放置和物理实现 |
| C6 | **更多决策需要在运行期完成**：部分有效统计信息只能在执行过程中观测 | 传统优化器主要在编译期选择计划；AI 算子还需要根据试跑或在线反馈自适应调整 |
| C7 | **质量约束缺少标准表达**：用户需要声明可接受的质量损失 | SQL 缺少表达近似正确性的标准语法；各系统主要通过自定义 API 提供该能力 |
| C8 | **优化开销不可忽略**：采样试跑和 AI oracle 判定都会产生模型调用成本 | 对于短查询，优化开销可能接近或超过优化所带来的收益 |
| C9 | **执行资源约束增加**：GPU 配额、推理并发度和 API rate limit 都会影响计划执行 | 传统资源模型主要考虑 CPU、内存和 I/O，难以直接表达模型服务的资源约束 |
| C10 | **可重复性与一致性语义需要重新定义**：缓存策略、模型更新和数据库事务语义相互影响 | 在非确定算子下，传统的可重复读语义难以直接保证 |
